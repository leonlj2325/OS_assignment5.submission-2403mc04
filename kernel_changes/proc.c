...
proc_freepagetable(pagetable_t pagetable, uint64 sz)
{
  ...
  // SHM_BASE is outside p->sz, so remove the mapping explicitly before
  // freewalk() traverses the page table. The physical page is shared.
  uvmunmap(pagetable, SHM_BASE, 1, 0);
...
kfork(void)
{
  ...
  // The shared page is intentionally outside p->sz, so uvmcopy() does not
  // duplicate it. Map the same physical page into the child instead.
  np->shm_pa = p->shm_pa;
  if (p->shm_pa != 0) {
    if (mappages(np->pagetable, SHM_BASE, PGSIZE, p->shm_pa,
                 PTE_R | PTE_W | PTE_U) < 0) {
      freeproc(np);
      release(&np->lock);
      return -1;
